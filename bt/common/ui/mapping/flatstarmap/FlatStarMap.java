...
//                comp2D.clipRect(0,0,MAPWIDTH+(MAPGUTTER*2),MAPWIDTH+(MAPGUTTER*2));
            ...
            comp2D.setFont(f);
            for (int j = 0; j < PlanetManager.getPlanetCount(); j++)
            {
                InnerSpherePlanet p = PlanetManager.getPlanet(j);
...
//                comp2D.drawArc(xoff-7,yoff-7,9 + (int)m_Zoom,9 + (int)m_Zoom,0,360);
        ...
        {
            InnerSpherePlanet p = PlanetManager.getPlanet(((Integer)m_SelectedSystems.elementAt(i)).intValue());
            int xoff = GetXOffset(p.getXCoord());
            int yoff = GetYOffset(p.getYCoord());
            ...
            Vector<SolarSystemRoute> v = PlanetManager.getRoutesForSystem(p);
            for (int j = 0; j < v.size(); j++)
            {
                SolarSystemRoute ssr = v.elementAt(j);
                InnerSpherePlanet dest = PlanetManager.getPlanetFromID(ssr.getDestinationSystem(p.getID()));
...
/*        if (m_CurrentSolarSystem != -1)
    ...
    protected double GetWidth()
    {
        ...
        if (m_Width == 0.0)
        {
            ...
            {
                InnerSpherePlanet m = PlanetManager.getPlanet(i);
                if (m.getXCoord() < minX)
                    minX = m.getXCoord();
                if (m.getXCoord() > maxX)
                    maxX = m.getXCoord();
    ...
    public int GetSystemIndex(Point p)
    {
        ...
        for (int j = 0; j < PlanetManager.getPlanetCount(); j++)
        {
            InnerSpherePlanet isp = PlanetManager.getPlanet(j);
    ...
    {
        return PlanetManager.getDistance(System1,System2);
    }

    protected Polygon ConvexHull()
    {
        Polygon p = new Polygon();
        Point points[] = new Point[PlanetManager.getPlanetCount()];
        for (int i = 0; i < PlanetManager.getPlanetCount(); i++)
        {
            InnerSpherePlanet isp = PlanetManager.getPlanet(i);
...
//            int M = GrahamScan(points,m_Systems.GetSystemCount()-1);
        int M = wrap(points,PlanetManager.getPlanetCount()-1);
        for (int i = 0; i <= M; i++)
