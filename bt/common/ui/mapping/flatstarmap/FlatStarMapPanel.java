...
import bt.common.ui.mapping.flatstarmap.FlatStarMapPane;
...
public class FlatStarMapPanel extends JPanel  implements ActionListener, MouseListener
{
    ...
    protected Vector<PlanetManagerListener> m_PlanetManagerListeners = new Vector<PlanetManagerListener>();

    public FlatStarMapPanel()
    {
        m_MapPanel = new JPanel(new BorderLayout());

        m_Map = new FlatStarMap();
    ...
    public void mouseReleased(MouseEvent e)
    {
        if ((e.getModifiers() & InputEvent.BUTTON1_MASK) > 0)
        {
            try
            {
                ...
                if (index != -1)
                {
                    ...
                    InnerSpherePlanet p1 = PlanetManager.getPlanet(index);
        ...
        if ((e.getModifiers() & InputEvent.BUTTON3_MASK) > 0)
        {
            ...
            if (i1 != -1 && i2 != -1)
            {
                ...
                try
                {
                    InnerSpherePlanet p1 = PlanetManager.getPlanet(i1);
                    InnerSpherePlanet p2 = PlanetManager.getPlanet(i2);
...
//        m_MapPanel.setBounds(x,y,w,h);
    ...
    public void addSolarSystemManagerListener(PlanetManagerListener sspml)
    {
        if (!m_PlanetManagerListeners.contains(sspml))
            m_PlanetManagerListeners.add(sspml);
    }

    public void removeSolarSystemManagerListener(PlanetManagerListener sspml)
    {
        if (m_PlanetManagerListeners.contains(sspml))
            m_PlanetManagerListeners.remove(sspml);
    }

    public void clearSolarSystemManagerListener()
    {
        m_PlanetManagerListeners.clear();
    }
    ...
    protected void NotifySolarSystemEditRequested(int Index)
    {
        ...
        for (int i = 0; i < m_PlanetManagerListeners.size(); i++)
        {
            PlanetManagerListener sspml = (PlanetManagerListener)m_PlanetManagerListeners.elementAt(i);
            sspml.PlanetEditRequest(isp);
        }
