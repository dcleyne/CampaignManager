...
import bt.common.elements.galaxy.Settlement;
...
{
    private WorldSector[] m_WorldSectors;
    ...
    public WorldMap(SolarSystemDetails details,int hexWidth)
    {
        m_Details = details;
    ...
    public WorldMap(PlanetMap map, int size, int hexWidth)
    {
        m_Details = null;
        m_Map = map;
        m_HexMap = new WorldMapNavigation();
    ...
    protected void InitialiseMap( int Size, int HexWidth )
    {
	    ...
	    m_WorldSectors = new WorldSector[(Size * Size * 10) + 2];
	
	    m_MapWidth = (HexX * (Size * 12)) + (BaseXOffset * 2);
	    m_MapHeight = (BaseYOffset * 2) + (m_HexMap.getMapRowHeight() * (HexY + 1));
	
		int CurHex = 1;
		m_WorldSectors[CurHex-1] = new WorldSector(BaseXOffset + (Size * HexX),BaseYOffset,HexWidth,m_Map.getPlanetSector(CurHex),m_HexMap);
		...
		for (int Segment = 0; Segment < 10; Segment++)
		{
			...
			for (int Row = 1; Row <= m_HexMap.getSegmentRows(); Row++)
			{
	                    ...
	                    {
	                        int NewOff = 0;
	                        m_WorldSectors[CurHex-1] = new WorldSector(HexOff.x,HexOff.y,HexWidth,m_Map.getPlanetSector(CurHex),m_HexMap);
		...
		m_WorldSectors[CurHex-1] = new WorldSector(BaseXOffset + ((Size * 2) * HexX),BaseYOffset + (m_HexMap.getMapRowHeight() * HexY) - HexY,HexWidth,m_Map.getPlanetSector(CurHex),m_HexMap);
    ...
    protected void drawMap(Graphics2D comp2D, Shape CurClip, DrawMode mode)
    {
        ...
        {
        	case MAP:
        		WorldMapTerrain.getInstance(); //Force the Image load;
        		WorldMapSettlement.getInstance(); //Force the Image load;
		        ...
		        if (m_Details != null)
		        {
			        for (int i = 0; i < m_Details.getSettlementCount(); i++)
			        {
			        	...
			        	WorldSector ws = m_WorldSectors[s.getLocation() - 1];
